...
class FetchUnitRME(params: RelMemParams, adapter: TLAdapterNode, tlInEdge: TLEdgeIn, instance: Int, subInstance: Int)(
    ...
    val io = IO(new FetchUnitIO(tlInEdge.bundle, tlOutParams, maxID)).suggestName(s"fetchunitio_$instance-$subInstance")
        ...
        when(io.OutReq.fire)
        {
            SynthesizePrintf("[FetchUnit_%d_%d] ==> fired request to DRAM src: %d baseReq 0x%x baseReqSrc %d, address 0x%x\n", instance.U, subInstance.U, io.OutReq.bits.source, baseReq.address, baseReq.source, io.OutReq.bits.address)
        }
